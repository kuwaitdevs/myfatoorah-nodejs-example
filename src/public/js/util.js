...
const putRequest = async ({ url, queryParams, data }) => {
	let finalUrl = "";
...
const deleteRequest = async ({ url, queryParams, data }) => {
	let finalUrl = "";
...
const showToast = ({ title, message }) => {
	const toastId = "id_" + GUID();

	let toastArea = document.getElementById("areaToast");

	if (toastArea === null) {
		const toastElement = document.createElement("div");
		toastElement.id = "areaToast";
		...
		toastElement.classList.add("p-3");
		document.body.appendChild(toastElement);
		toastArea = document.getElementById("areaToast");
	}
